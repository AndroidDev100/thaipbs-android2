...
import com.mvhub.mvhubplus.databinding.LayoutDownloadQualityBottomSheetBinding
import com.mvhub.mvhubplus.databinding.WifiDialogBinding
import com.mvhub.mvhubplus.utils.constants.SharedPrefesConstants
import com.mvhub.mvhubplus.utils.helpers.downloads.BrightcoveDownloadUtil
import com.mvhub.mvhubplus.utils.helpers.downloads.ImageDownloadHelper
import com.mvhub.mvhubplus.utils.helpers.downloads.room.DownloadDatabase
...
import com.mvhub.mvhubplus.utils.helpers.downloads.room.DownloadedVideo
...
class DownloadHelper() {
    ...
    private lateinit var videoListListener: com.mvhub.mvhubplus.utils.helpers.downloads.VideoListListener
    ...
    private lateinit var enveuVideoItemBean: com.mvhub.mvhubplus.beanModelV3.uiConnectorModelV2.EnveuVideoItemBean
    ...
    constructor(activity: Activity) : this() {
        ...
        db = Room.databaseBuilder(
                com.mvhub.mvhubplus.MvHubPlusApplication.getApplicationContext(activity),
    ...
    constructor(activity: Activity, videoListener: MediaDownloadable.DownloadEventListener) : this() {
        this.videoListListener = videoListener as com.mvhub.mvhubplus.utils.helpers.downloads.VideoListListener
    ...
    constructor(activity: Activity, videoListener: MediaDownloadable.DownloadEventListener, seriesId: String, seriesName: String, assetType: String, enveuVideoItemBean: com.mvhub.mvhubplus.beanModelV3.uiConnectorModelV2.EnveuVideoItemBean) : this(activity, videoListener) {
        this.assetType = assetType
    ...
    fun allowedMobileDownload() {
        mobileDownloadAllwed = com.mvhub.mvhubplus.utils.helpers.ksPreferenceKeys.KsPreferenceKeys.getInstance().downloadOverWifi
        catalog.isMobileDownloadAllowed = mobileDownloadAllwed==0
    }

    fun findVideo(videoId: String): Video? {
        com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, "VideoId$videoId")
        catalog.findVideoByID(videoId, object : VideoListener() {
            ...
            override fun onError(error: String) {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, error)
            }
        })
        return video
    }
    ...
    fun startVideoDownload(video: Video, videoQuality: Int) {
        if (video.isClearContent) {
            ...
            acquireLicense(video, object : com.mvhub.mvhubplus.callbacks.commonCallbacks.CommonApiCallBack {
                override fun onSuccess(item: Any?) {
                    downloadVideo(item as Video, videoQuality)
                }

                override fun onFailure(throwable: Throwable?) {
                    com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, throwable?.message)
                }
    ...
    fun startEpisodeDownload(video: Video, seriesId: String, seasonNumber: Int, episodeNumber: String, videoQuality: Int) {
        assetType = com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().episode
        this.seriesId = seriesId
    ...
    private fun acquireLicense(video: Video, param: com.mvhub.mvhubplus.callbacks.commonCallbacks.CommonApiCallBack) {
        var playDuration = (video.duration.plus(10000)).toLong()
        com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, playDuration.toString())
        ...
        catalog.requestRentalLicense(video, calender.time, calender.timeInMillis, { event ->
            when (event.type) {
                EventType.ODRM_LICENSE_ACQUIRED -> {
                    com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.i(TAG, "ODRM_LICENSE_ACQUIRED" + Gson().toJson(event))
                    param.onSuccess(event.properties[Event.VIDEO] as Video)
                }
                EventType.ODRM_PLAYBACK_NOT_ALLOWED, EventType.ODRM_SOURCE_NOT_FOUND -> {
                    com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.w(TAG, "ODRM_PLAYBACK_NOT_ALLOWED")
                    param.onFailure(Throwable("ODRM_PLAYBACK_NOT_ALLOWED"))
                }
                EventType.ODRM_LICENSE_ERROR -> {
                    com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, "ODRM_LICENSE_ERROR" + event.properties[Event.ERROR] as Throwable?)
...
//                cancelVideo(videoId)
    ...
    fun deleteVideo(video: Video) {
        catalog.deleteVideo(video, object : OfflineCallback<Boolean> {
            override fun onSuccess(aBoolean: Boolean?) {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, aBoolean!!.toString())
                if (aBoolean) {
                    checkDownloadStatus()
    ...
    private fun downloadVideo(video: Video, videoQuality: Int) {
        ...
        when (videoQuality) {
            0 -> {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("Download", BEST_BIT_RATE.toString())
                catalog.setVideoBitrate(BEST_BIT_RATE)
            }
            1 -> {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("Download", BETTER_BIT_RATE.toString())
                catalog.setVideoBitrate(BETTER_BIT_RATE)
            }
            2 -> {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("Download", GOOD_BIT_RATE.toString())
                catalog.setVideoBitrate(GOOD_BIT_RATE)
            }
            3 -> {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("Download", DATA_SAVER_BIT_RATE.toString())
                catalog.setVideoBitrate(DATA_SAVER_BIT_RATE)
            }
        }
        if (assetType != com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().series) {
            catalog.getMediaFormatTracksAvailable(video) { mediaDownloadable, bundle ->
                ...
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("Bundle", Gson().toJson(filteredBundle))
                catalog.downloadVideo(video, object : OfflineCallback<DownloadStatus> {
                    ...
                    override fun onFailure(throwable: Throwable) {
                        com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, "onFailure====>" + throwable.message)
                    }
                })
            }
        }
    ...
    private fun addVideotoDatabase(video: Video, seriesId: String) {
        if (assetType == com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().series || assetType == com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().episode) {
            findVideo(seriesId, object : VideoListener() {
                override fun onVideo(p0: Video?) {
                    var downloadedVideo = DownloadedVideo(p0!!.id, com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().series, seriesId)
                    ...
                    ImageDownloadHelper(activity as Context, seriesId, object : com.mvhub.mvhubplus.callbacks.commonCallbacks.CommonApiCallBack {
                        override fun onSuccess(item: Any?) {
                            insertVideo(downloadedVideo, downloadedEpisodes)
                        }

                        override fun onFailure(throwable: Throwable?) {
                            com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, "Unable to save image")
                            insertVideo(downloadedVideo, downloadedEpisodes)
            ...
            var downloadedVideo = DownloadedVideo(video.id, assetType, seriesId, "", "", video.name, com.mvhub.mvhubplus.utils.commonMethods.AppCommonMethod.expiryDate(3))
            insertVideo(downloadedVideo, null)
        }

        if (assetType == com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().episode || assetType == com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().series) {
            try {
                AsyncTask.execute {
                    val downloadedEpisodes = DownloadedEpisodes(video.id, seasonNumber!!, episodeNumber!!, seriesId)
                ...
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, ex.message)
            }
        }
    }
    ...
    private fun deleteVideoFromDatabase(videoId: String) {
        com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("DeleteVideo",videoId)
        try {
            AsyncTask.execute {
        ...
        video?.id?.let {
            catalog.pauseVideoDownload(it, object : OfflineCallback<Int> {
                override fun onSuccess(p0: Int?) {
                    com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, "pauseDownload$p0")
                }
        ...
        findVideo(videoId, object : VideoListener() {
            override fun onVideo(p0: Video?) {
                if (p0?.isClearContent!!) {
                    ...
                    acquireLicense(p0, object : com.mvhub.mvhubplus.callbacks.commonCallbacks.CommonApiCallBack {
                        override fun onSuccess(item: Any?) {
    ...
    fun getDownloadedVideos() {
        catalog.findAllVideoDownload(DownloadStatus.STATUS_COMPLETE, object : OfflineCallback<List<Video>> {
            override fun onSuccess(p0: List<Video>?) {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e(TAG, Gson().toJson(p0?.get(1)))
    ...
    fun startSeriesDownload(seriesId: String, selectedSeason: Int, seasonEpisodes: MutableList<out com.mvhub.mvhubplus.beanModelV3.uiConnectorModelV2.EnveuVideoItemBean>, videoQuality: Int) {
    ...
    fun selectVideoQuality(videoQualitySelectedListener: com.mvhub.mvhubplus.activities.downloads.VideoQualitySelectedListener) {
        ...
        val changeDownloadQualityAdapter = com.mvhub.mvhubplus.activities.downloads.SelectDownloadQualityAdapter(activity, downloadQualityList, object : com.mvhub.mvhubplus.activities.downloads.VideoQualitySelectedListener {
        ...
        binding.btnStartDownload.setOnClickListener {
            if (binding.checkboxMakeDefault.isChecked) {
                com.mvhub.mvhubplus.utils.helpers.SharedPrefHelper(activity).setInt(SharedPrefesConstants.DOWNLOAD_QUALITY_INDEX, selectedVideoQualityPosition)
            }
    ...
    fun changeWifiSetting(videoQualitySelectedListener: com.mvhub.mvhubplus.activities.downloads.WifiPreferenceListener) {
        ...
        downloadQualityList.addAll(activity.resources.getStringArray(R.array.download_quality))
        downloadQualityList.removeAt(4)
        binding.switchDownload.isChecked= com.mvhub.mvhubplus.utils.helpers.ksPreferenceKeys.KsPreferenceKeys.getInstance().downloadOverWifi == 1

        binding.btnStartDownload.setOnClickListener {
            if (binding.switchDownload.isChecked) {
                com.mvhub.mvhubplus.utils.helpers.ksPreferenceKeys.KsPreferenceKeys.getInstance().downloadOverWifi = 1
                //SharedPrefHelper(activity).setInt(SharedPrefesConstants.DOWNLOAD_QUALITY_INDEX, selectedVideoQualityPosition)
            }else{
                com.mvhub.mvhubplus.utils.helpers.ksPreferenceKeys.KsPreferenceKeys.getInstance().downloadOverWifi=0
            }
            com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("DownloadedVideo1", Gson().toJson(com.mvhub.mvhubplus.utils.helpers.ksPreferenceKeys.KsPreferenceKeys.getInstance().downloadOverWifi))
            videoQualitySelectedListener.actionP(com.mvhub.mvhubplus.utils.helpers.ksPreferenceKeys.KsPreferenceKeys.getInstance().downloadOverWifi)
            dialog.dismiss()
        }
    ...
    fun deleteAllExpiredVideos() {
        ...
        getAllVideosFromDatabase().observe(activity as LifecycleOwner, androidx.lifecycle.Observer {
            it.downloadVideos.forEach { downloadedVideo ->
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("DownloadedVideo1", Gson().toJson(downloadedVideo))
                if (downloadedVideo.downloadType != com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().series) {
                            ...
                            if (video != null) {
                                if (!video.isClearContent) {
                                    if (video.licenseExpiryDate!!.time >= System.currentTimeMillis()) {
                                        com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.e("License", "Expiry" + video.licenseExpiryDate)
                                    } else {
                                        deleteVideo(video)
                                    }
                                }else{
    ...
    private fun deleteExpireVideoFromDB(videoId: DownloadedVideo) {
        val diff: Int = com.mvhub.mvhubplus.utils.commonMethods.AppCommonMethod.getTodaysDifference(videoId.expiryDate)
        if (diff == 1) {
            try {
    ...
    private class DownloadTask : AsyncTask<DownloadDatabase, Any, DownloadModel>() {
        ...
        override fun doInBackground(vararg params: DownloadDatabase): DownloadModel {
            ...
            seriesId.forEachIndexed { index, downloadVideo ->
                ...
                if (downloadVideo.downloadType == com.mvhub.mvhubplus.utils.MediaTypeConstants.getInstance().series && downloadedEpisodes.size > 0) {
