...
object BrightcoveDownloadUtil {
    ...
    fun selectMediaFormatTracksAvailable(mediaDownloadable: MediaDownloadable, bundle: Bundle) {
        ...
        if (audio != null && audio.size > 0) {
            com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Adding the \"main\" audio track.")
        ...
        if (audio!!.size > 1) {
            com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Alternate audio track download allowed for this video. Adding an \"alternate\" audio track")
            if (indexMain == 0) {
            ...
            com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Alternate audio track download allowed, but there were no \"alternate\" audio tracks to select.")
        }
        ...
        com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Captions array size: " + captions!!.size)
        if (captions.size > 0) {
            val newCaptions = ArrayList<MediaFormat>()
            com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Adding the first caption track as the \"default\" caption track.")
            newCaptions.add(captions[0])

            if (captions.size > 1) {
                com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Adding the first of the remaining caption tracks as the \"alternate\" caption track.")
                newCaptions.add(captions[1])
            } else {
               com.mvhub.mvhubplus.utils.cropImage.helpers.Logger.v(TAG, "Captions size is not GT 1, no alternate captions will be downloaded, even though allowed.")
            }
