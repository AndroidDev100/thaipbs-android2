package com.mvhub.mvhubplus.networking.servicelayer;
...
import com.mvhub.mvhubplus.utils.config.ImageLayer;
                            ...
                            for (int i = 0; i < data.size(); i++) {
                                ...
                                if (data != null) {
                                    ...
                                    if (data.get(i).getData() != null && data.get(i).getData().getItems() != null) {
                                        railCommonData.setStatus(true);
                                        List<com.mvhub.mvhubplus.beanModelV3.searchV2.ItemsItem> searchItems = data.get(i).getData().getItems();
                                        List<EnveuVideoItemBean> enveuVideoItemBeans = new ArrayList<>();
                                        for (com.mvhub.mvhubplus.beanModelV3.searchV2.ItemsItem videoItem : searchItems) {
                                            Gson gson = new Gson();
                    ...
                    if (data.code() == 200) {
                        ...
                        if (data != null) {
                            ...
                            if (data.body().getData() != null && data.body().getData().getItems() != null) {
                                railCommonData.setStatus(true);
                                List<com.mvhub.mvhubplus.beanModelV3.searchV2.ItemsItem> itemsItem = data.body().getData().getItems();
                                enveuVideoItemBeans = new ArrayList<>();
                                for (com.mvhub.mvhubplus.beanModelV3.searchV2.ItemsItem videoItem : itemsItem) {
                                    EnveuVideoItemBean enveuVideoItemBean = new EnveuVideoItemBean(videoItem);
                                    enveuVideoItemBean.setPosterURL(ImageLayer.getInstance().getPosterImageUrl(videoItem));
