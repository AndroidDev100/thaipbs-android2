package com.mvhub.mvhubplus.repository.sinetron;
...
public class SinetronFragmentRepository {

    private static SinetronFragmentRepository projectRepository;
    ...
    public synchronized static SinetronFragmentRepository getInstance() {
        if (projectRepository == null) {
            projectRepository = new SinetronFragmentRepository();
        }
    ...
    private void getRailData(List<PlaylistsItem> playlist, MutableLiveData<List<CommonRailData>> playlistResponsesMutableLiveData) {
        ...
        if (count != playlist.size()) {
            ...
            for (int i = 0; i < commonRailData.size(); i++) {
                int headerItemLimit = 5;
                commonData = new CommonRailData();
                if (i == 0) {
                    ArrayList<Slide> slides = new ArrayList<>();
                    if (commonRailData.size() < 5) {
                        headerItemLimit = commonRailData.size();
                    }

                    for (int j = 0; j < headerItemLimit; j++) {
                        Slide slide = new Slide();
                        commonData.setType(0);
                        slide.setType(1);
                        if (commonRailData.get(i).getData().getContentType().equalsIgnoreCase(AppConstants.VOD)) {
                            slide.setImageFromUrl(AppCommonMethod.urlPoints + AppConstants.VIDEO_IMAGE_BASE_KEY + commonRailData.get(i).getData().getContents().get(j).getLandscapeImage());
                        } else if (commonRailData.get(i).getData().getContentType().equalsIgnoreCase(AppConstants.SERIES)) {
                            slide.setImageFromUrl(AppCommonMethod.urlPoints + AppConstants.SERIES_IMAGES_BASE_KEY + commonRailData.get(i).getData().getContents().get(j).getPicture());
                        } else if (commonRailData.get(i).getData().getContentType().equalsIgnoreCase(AppConstants.CAST_AND_CREW)) {
                            slide.setImageFromUrl(AppCommonMethod.urlPoints + AppConstants.CAST_CREW_IMAGES_BASE_KEY + commonRailData.get(i).getData().getContents().get(j).getPicture());
                        } else if (commonRailData.get(i).getData().getContentType().equalsIgnoreCase(AppConstants.GENRE)) {
                            slide.setImageFromUrl(AppCommonMethod.urlPoints + AppConstants.GENRE_IMAGES_BASE_KEY + commonRailData.get(i).getData().getContents().get(j).getPicture());
                        }
